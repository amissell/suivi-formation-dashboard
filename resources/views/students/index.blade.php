...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="text-muted-foreground">Gérez les étudiants de votre académie</p>
        </div>
        <!-- Search by name -->
    ...
    <div class="bg-card border border-border rounded-lg shadow-sm">
        ...
        <div class="p-6 border-b border-border">
            <form method="GET" class="flex flex-wrap items-center gap-3">
                ...
                <!-- Status filter -->
                <select name="formation_id" class="border border-border rounded-md p-2">
        ...
        <div class="p-6 overflow-x-auto">
            <table class="w-full table-auto">
                ...
                <tr class="border-b border-border">
                    <th class="text-left text-sm font-medium text-muted-foreground pb-3">Nom</th>
                    <th class="text-left text-sm font-medium text-muted-foreground pb-3">CIN</th>
                    <th class="text-left text-sm font-medium text-muted-foreground pb-3">Téléphone</th>
                    <th class="text-left text-sm font-medium text-muted-foreground pb-3">E-mail</th>
                    <th class="text-left text-sm font-medium text-muted-foreground pb-3">Formation</th>
                    <th class="text-left text-sm font-medium text-muted-foreground pb-3">Date de Début</th>
                    <th class="text-left text-sm font-medium text-muted-foreground pb-3">Statut</th>
                    <th class="text-left text-sm font-medium text-muted-foreground pb-3">Attestation</th>
                    <th class="text-left text-sm font-medium text-muted-foreground pb-3">Payé</th>
                    <th class="text-left text-sm font-medium text-muted-foreground pb-3">Reste</th>
                    <th class="text-left text-sm font-medium text-muted-foreground pb-3">Ville</th>
                ...
                <tr class="border-b border-border last:border-0">
                    ...
                    <td class="py-4 text-right flex justify-end gap-2">
                        ...
                        <form action="{{ route('students.destroy', $student) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" onclick="return confirm('Êtes-vous sûr ?')" class="w-9 h-9 flex items-center justify-center rounded-full border border-border hover:bg-red-100 hover:text-red-600 transition">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path d="M19 7L5 7" />
